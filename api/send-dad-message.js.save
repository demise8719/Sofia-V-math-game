...
module.exports = async function handler(req, res) {
  ...
  try {
    const { message, from, lang } = req.body || {};
    ...
    const cleanMessage = String(message).slice(0, 500).replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g, '');
    const senderName = (from ? String(from) : 'Sofia').slice(0, 40);
